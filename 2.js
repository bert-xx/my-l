(function () {
    ...
    // 2. ЖАНРЫ
    var movieGenres = [
        { title: 'Комедии', id: 35 }, { title: 'Ужасы', id: 27 }, { title: 'Боевики', id: 28 }, 
    ...
    // 4. ГЕНЕРАЦИЯ ПЕРЕХОДА (ИСПРАВЛЕНО ДЛЯ СЕТКИ)
    function showGenres(title, genres, type) {
        ...
        Lampa.Select.show({
            ...
            items: items,
            onSelect: function (a) {
                // Для CUB и TMDB в Лампе сетка жанров вызывается через правильный тип компонента
                // Мы передаем 'id' жанра и указываем, что это список (фильтрация)
                Lampa.Activity.push({
                    url: type, // 'movie' или 'tv'
                    title: a.title,
                    component: type, // 'movie' или 'tv' открывает сетку
                    id: a.id,
                    source: currentSource,
                    filter: {
                        genre: a.id // Явно передаем фильтр по жанру
                    },
                    page: 1
                });
            },
