(function () {
  ...
  var Defined = {
    api: 'lampac',
    localhost: 'https://o.1c-do.website/',
    apn: ''
  };
  ...
  var hostkey = 'https://o.1c-do.website'.replace('http://', '').replace('https://', '');
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
        ...
        net.silent('https://o.1c-do.website'.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
          check(true);
        }, function() {
          check(false);
        }, false, { dataType: 'text' });
  ...
  window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
    window.rch_nws[hostkey].typeInvoke('https://o.1c-do.website', function() {
      ...
      client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
        ...
        function sendResult(uri, html) {
          $.ajax({
            url: 'https://o.1c-do.website/rch/' + uri + '?id=' + rchId,
        ...
        function result(html) {
          ...
          if (typeof CompressionStream !== 'undefined' && html && html.length > 1000) {
            ...
            var readable = new ReadableStream({
              start: function(controller) { controller.enqueue(encoder.encode(html)); controller.close(); }
            });
        ...
        if (url == 'eval') { console.log('RCH', url, data); result(eval(data)); }
          ...
          console.log('RCH', url);
          network["native"](url, result, function(e) { console.log('RCH', 'result empty, ' + e.status); result(''); }, data, {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript(["https://o.1c-do.website/js/nws-client-es5.js?v21042026"], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
  ...
  // ═══════════════════════════════════════════════════════════
  // LUMIO DESIGN: функции для цветовых тонов
  // ═══════════════════════════════════════════════════════════
  function lumioStableIndex(value, max) {
  ...
  function lumioVoiceTone(text) {
    var t = String(text || '').toLowerCase();
    var base = 'voice';
    if (/субтитр|sub/.test(t)) base = 'subs';
    else if (/оригинал|original|orig|\(ор\)/.test(t)) base = 'orig';
    else if (/дубл|dub|лиценз/.test(t)) base = 'dub';
    else if (/многогол|мво|multi/.test(t)) base = 'mvo';
    else if (/авторск|аво|гавр|волод|кухар/.test(t)) base = 'avo';
    else if (/lostfilm|newstudio|hdrezka|coldfilm|baibako|le[- ]?prod|idea|alex/.test(t)) base = 'studio';
    return lumioStableIndex(base + ':' + t.slice(0, 40), 12);
  }

  function lumioEpisodeTone(season) {
    var s = parseInt(season || 1, 10) || 1;
    return (s % 4) + 1;
  }

  function lumioQualityClass(text) {
    var t = String(text || '').toLowerCase();
    if (/2160|4k|uhd/.test(t)) return 'lumio-quality--4k';
    if (/1440|2k/.test(t)) return 'lumio-quality--2k';
    if (/1080|fhd|fullhd/.test(t)) return 'lumio-quality--1080';
    if (/720|hd/.test(t)) return 'lumio-quality--720';
    if (/480|360|240|sd/.test(t)) return 'lumio-quality--sd';
    return '';
  }

  // Применение тонов ко всем карточкам online-prestige в контейнере
  function applyLumioTones(container) {
    if (!container || !container.length) return;
    container.find('.online-prestige').each(function() {
      var card = $(this);
      if (card.attr('data-lumio')) return;
      card.attr('data-lumio', '1');

      // Добавляем glow-эффект
      if (!card.find('.online-prestige__glow').length) {
        card.prepend('<div class="online-prestige__glow"></div>');
      }
      ...
      var epNum = card.find('.online-prestige__episode-number');

      if (img.length) {
        // Очищаем предыдущие тона
        for (var i = 0; i < 12; i++) img.removeClass('lumio-voice-tone-' + i);
        for (var j = 1; j <= 4; j++) img.removeClass('lumio-episode-tone-' + j);

        if (epNum.length) {
          // Эпизод: тон по номеру сезона из info
          var infoText = info.text() || '';
          var sm = infoText.match(/(\d+)\s*(?:сезон|season)/i);
          var season = sm ? parseInt(sm[1], 10) : 1;
          img.addClass('lumio-episode-tone-' + lumioEpisodeTone(season));
        } else {
          // Фильм/озвучка: тон по тексту озвучки
          var voiceText = info.text() || '';
          img.addClass('lumio-voice-tone-' + lumioVoiceTone(voiceText));
        }
      }

      // Цветной бейдж качества
      if (quality.length) {
        var qText = quality.text() || '';
        quality.removeClass('lumio-quality--4k lumio-quality--2k lumio-quality--1080 lumio-quality--720 lumio-quality--sd');
        var qClass = lumioQualityClass(qText);
        if (qClass) quality.addClass(qClass);
      }
    });
  }

  // ═══════════════════════════════════════════════════════════
  // КОМПОНЕНТ (механизм без изменений)
  // ═══════════════════════════════════════════════════════════
  function component(object) {
    ...
    if (balansers_with_search == undefined) {
      ...
      network.silent(
        account('https://o.1c-do.website/lite/withsearch'),
        function (json) { balansers_with_search = json; },
    ...
    this.initialize = function () {
      ...
      filter.render().find('.selector').on('hover:enter', function () { stopBalanserTimer(); });
      ...
      if (object.balanser) {
        ...
        return network['native'](
          ...
          function () { files.render().find('.torrent-filter').remove(); _this.empty(); },
          false,
    ...
    this.rch = function (json, noreset) {
      var _this2 = this;
      rchRun(json, function () { if (!noreset) _this2.find(); else noreset(); });
    };
    ...
    this.createSource = function () {
      ...
      return new Promise(function (resolve, reject) {
        ...
        network.silent(account(url), function (json) {
          ...
          } else _this4.startSource(json).then(resolve)['catch'](reject);
    ...
    this.request = function (url) {
      ...
      if (number_of_requests < 10) {
        ...
        number_requests_timer = setTimeout(function () { number_of_requests = 0; }, 4000);
      } else this.empty();
    };
        ...
        network['native'](account(file.url), function (json) {
          if (json.rch) {
            if (waiting_rch) { waiting_rch = false; Lampa.Loading.stop(); call(false, {}); }
              ...
              _this.rch(json, function () { Lampa.Loading.stop(); _this.getFileUrl(file, call, true); });
            }
    ...
    this.toPlayElement = function (file) {
      return {
        title: file.title, url: file.url, quality: file.qualitys,
        timeline: file.timeline, subtitles: file.subtitles,
        segments: file.segments, callback: file.mark,
        season: file.season, episode: file.episode,
        voice_name: file.voice_name, thumbnail: file.thumbnail
      };
    };
    ...
    this.similars = function (json) {
      ...
      // ═══ LUMIO: применяем тона к похожим карточкам ═══
      applyLumioTones(scroll.render());
      ...
      this.getEpisodes(items[0].season, function (episodes) {
        ...
        items.forEach(function (element, index) {
          ...
          var hash_timeline = Lampa.Utils.hash(
            element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title].join('') : object.movie.original_title
          );
          var hash_behold = Lampa.Utils.hash(
            element.season ? [element.season, element.season > 10 ? ':' : '', element.episode, object.movie.original_title, element.voice_name].join('') : object.movie.original_title + element.voice_name
          );
          ...
          if (episode) {
            ...
            if (element.info.length < 30 && episode.vote_average) info.push(
              Lampa.Template.get('lampac_prestige_rate', { rate: parseFloat(episode.vote_average + '').toFixed(1) }, true)
            );
        ...
        // ═══ LUMIO: применяем тона ко всем карточкам ═══
        applyLumioTones(scroll.render());
    ...
    this.doesNotAnswer = function (er) {
      ...
      html.find('.cancel').on('hover:enter', function () { stopBalanserTimer(); });
      html.find('.change').on('hover:enter', function () { stopBalanserTimer(); filter.render().find('.filter--sort').trigger('hover:enter'); });
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    ...
    this.background = function () { Lampa.Background.immediately(Lampa.Utils.cardImgBackgroundBlur(object.movie)); };
    ...
    this.start = function () {
      ...
      Lampa.Controller.add('content', {
        ...
        gone: function gone() { stopBalanserTimer(); },
    ...
    function suspendActivity() { stopBalanserTimer(); clearTimeout(life_wait_timer); clearTimeout(number_of_requests_timer); network.clear(); }
    this.pause = function () { suspendActivity(); };
    this.stop = function () { suspendActivity(); };
    ...
    var source = {
            ...
            status.onComplite = function (result) {
              ...
              keys.forEach(function (name) {
                ...
                if (line && line.data && line.type == 'similar') {
                  ...
                  rows.push(Lampa.Utils.addSource({ title: name, results: cards }, 'lampac'));
                }
              });
              oncomplite(rows);
            };
      ...
      onSelect: function (params, close) {
        ...
        Lampa.Activity.push({
          ...
          component: 'lampac', movie: params.element, page: 1,
          search: params.element.title, clarification: true,
          balanser: params.element.balanser, noinfo: true
        });
      }
    };
  ...
  function startPlugin() {
    ...
    var manifest = {
      type: 'video', version: LAMPAC_VERSION, name: 'Lampac',
      description: 'Плагин для просмотра онлайн сериалов и фильмов',
      component: 'lampac',
      onContextMenu: function onContextMenu(object) { return { name: Lampa.Lang.translate('lampac_watch'), description: '' }; },
    ...
    // ═══════════════════════════════════════════════════════════
    // LUMIO CSS — стиль Lumio применён к .online-prestige
    // ═══════════════════════════════════════════════════════════
    function lampacCssHtml() {
      return [
        ...
        /* ── Базовая карточка (стиль Lumio) ── */
        '.online-prestige {',
        '  position: relative;',
        '  overflow: hidden;',
        '  border-radius: 0.55em;',
        '  background: linear-gradient(110deg, rgba(15,23,42,.78), rgba(7,11,22,.52));',
        '  border: 1px solid rgba(255,255,255,.12);',
        '  box-shadow: 0 .45em 1.2em rgba(0,0,0,.28);',
        '  display: flex;',
        '  min-height: 7.4em;',
        '  transition: background .22s ease, transform .22s ease, box-shadow .22s ease;',
        '}',

        /* ── Свечение в углу (glow) ── */
        ...
        '  color: #fff;',
        '  z-index: 1;',
        '  text-shadow: 0 2px 12px rgba(0,0,0,.6), 0 0 2em rgba(18,214,223,.35);',
        '}',
        ...
        '  filter: drop-shadow(0 0 .4em rgba(18,214,223,.5));',
        '}',
        ...
        /* ── Timeline ── */
        '.online-prestige__timeline { margin: 0.6em 0; }',
        ...
        '  white-space: nowrap;',
        '}',
        '.online-prestige__time:empty { display: none; }',
        ...
        '  color: #9dffc0;',
        '  text-shadow: 0 0 .6em rgba(34,197,94,.4);',
        '}',
        ...
        '  background: linear-gradient(110deg, rgba(18,214,223,.2), rgba(155,92,255,.22)), rgba(7,11,22,.82);',
        '  box-shadow: 0 .6em 1.8em rgba(18,214,223,.25), 0 0 0 .18em rgba(255,255,255,.3);',
        '  transform: translateY(-2px);',
        '}',
        '.online-prestige.focus::after {',
        '  content: "";',
        '  position: absolute;',
        '  top: -0.28em; left: -0.28em;',
        '  right: -0.28em; bottom: -0.28em;',
        ...
        '.online-prestige--folder .online-prestige__footer { margin-top: 0.85em; }',
        ...
        '.online-empty__title { font-size: 1.8em; margin-bottom: 0.3em; }',
        ...
        '.online-empty__templates .online-empty-template:nth-child(2) { opacity: 0.5; }',
        '.online-empty__templates .online-empty-template:nth-child(3) { opacity: 0.2; }',
        ...
        /* ═══ LUMIO: цветовые тона озвучек ═══ */
        '.online-prestige__img.lumio-voice-tone-0 { box-shadow: inset 0 0 0 9999px rgba(18,214,223,.18), inset 0 0 3em rgba(155,92,255,.22); }',
        '.online-prestige__img.lumio-voice-tone-1 { box-shadow: inset 0 0 0 9999px rgba(34,197,94,.18), inset 0 0 3em rgba(18,214,223,.2); }',
        '.online-prestige__img.lumio-voice-tone-2 { box-shadow: inset 0 0 0 9999px rgba(244,114,182,.18), inset 0 0 3em rgba(155,92,255,.22); }',
        '.online-prestige__img.lumio-voice-tone-3 { box-shadow: inset 0 0 0 9999px rgba(250,204,21,.18), inset 0 0 3em rgba(34,197,94,.2); }',
        '.online-prestige__img.lumio-voice-tone-4 { box-shadow: inset 0 0 0 9999px rgba(96,165,250,.18), inset 0 0 3em rgba(18,214,223,.2); }',
        '.online-prestige__img.lumio-voice-tone-5 { box-shadow: inset 0 0 0 9999px rgba(248,113,113,.18), inset 0 0 3em rgba(250,204,21,.2); }',
        '.online-prestige__img.lumio-voice-tone-6 { box-shadow: inset 0 0 0 9999px rgba(45,212,191,.18), inset 0 0 3em rgba(96,165,250,.2); }',
        '.online-prestige__img.lumio-voice-tone-7 { box-shadow: inset 0 0 0 9999px rgba(192,132,252,.18), inset 0 0 3em rgba(244,114,182,.2); }',
        '.online-prestige__img.lumio-voice-tone-8 { box-shadow: inset 0 0 0 9999px rgba(251,146,60,.18), inset 0 0 3em rgba(248,113,113,.2); }',
        '.online-prestige__img.lumio-voice-tone-9 { box-shadow: inset 0 0 0 9999px rgba(74,222,128,.18), inset 0 0 3em rgba(250,204,21,.2); }',
        '.online-prestige__img.lumio-voice-tone-10 { box-shadow: inset 0 0 0 9999px rgba(56,189,248,.18), inset 0 0 3em rgba(129,140,248,.2); }',
        '.online-prestige__img.lumio-voice-tone-11 { box-shadow: inset 0 0 0 9999px rgba(217,70,239,.18), inset 0 0 3em rgba(34,211,238,.2); }',

        /* ═══ LUMIO: цветовые тона эпизодов ═══ */
        '.online-prestige__img.lumio-episode-tone-1 { box-shadow: inset 0 0 0 9999px rgba(34,197,94,.22), inset 0 0 3em rgba(18,214,223,.18); }',
        '.online-prestige__img.lumio-episode-tone-2 { box-shadow: inset 0 0 0 9999px rgba(155,92,255,.24), inset 0 0 3em rgba(244,114,182,.18); }',
        '.online-prestige__img.lumio-episode-tone-3 { box-shadow: inset 0 0 0 9999px rgba(250,204,21,.22), inset 0 0 3em rgba(249,115,22,.18); }',
        '.online-prestige__img.lumio-episode-tone-4 { box-shadow: inset 0 0 0 9999px rgba(18,214,223,.22), inset 0 0 3em rgba(79,70,229,.18); }',

        /* ── Mobile ── */
        '@media screen and (max-width: 480px) {',
        '  .online-prestige__body { padding: 0.85em 1em; }',
        '  .online-prestige__title { font-size: 1.25em; }',
    ...
    // ═══════════════════════════════════════════════════════════
    // LUMIO: обновлённые шаблоны (с glow-элементом)
    // ═══════════════════════════════════════════════════════════
    ...
    var LAMPAC_FULL_CARD_BUTTON = [
      '<div class="full-start__button selector view--online lampac--button" data-subtitle="' + manifest.name + ' v' + manifest.version + '">',
    ...
    if (Lampa.Manifest.app_digital >= 177) {
      var balansers_sync = [
        ...
        'iptvonline','veoveo','kinoflix','leproduction','vkmovie','kinogo','kinobase',
